 ...
 const ComparisonColumn: React.FC<ComparisonColumnProps> = ({ title, features, variant, className }) => {
  ...
  return (
    <div className={`flex flex-col ${isDark ? 'bg-[#00224D] text-white' : 'bg-white'} ${className}`}>
      <h4 className={`flex-1 shrink gap-1 self-stretch px-8 py-9 w-full ${isDark ? 'text-[28px] font-bold' : 'font-semibold'} text-center ${isDark ? 'bg-[#00224D] rounded-t-[20px]' : 'bg-white rounded-tr-[20px]'} min-h-[94px] max-md:px-5 max-md:max-w-full`}>
        {title}
      </h4>
